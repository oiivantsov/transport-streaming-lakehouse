...
rundate = get_rundate()
schema_name = "hdw_stg"
table_name = "stops_stg"
table_full_name = f"{schema_name}.{table_name}"
landing_table_full_name = "hdw_ld.stops_ld"
print("SPARK_APP: JOB triggered for rundate - " + rundate)

spark = (
    SparkSession
    .builder
    .appName("Stops Staging")
    .enableHiveSupport()
    .getOrCreate()
)
...
# Cast and clean columns according to requirements
df_transformed = (
    df_ld
    .withColumn("stop_lat", F.col("stop_lat").cast(DoubleType()))
    .withColumn("stop_lon", F.col("stop_lon").cast(DoubleType()))
    .drop("stop_url")  # remove
    .drop("location_type")  # remove
    .drop("_corrupt_record")  # remove
    .withColumn("parent_station", F.coalesce("parent_station", F.lit("N/A")))
    .withColumn("wheelchair_boarding", F.col("wheelchair_boarding").cast(IntegerType()))
    .withColumn("platform_code", F.coalesce("platform_code", F.lit("N/A")))
    .withColumn("vehicle_type", F.coalesce("vehicle_type", F.lit("N/A")))
    .withColumn("radius", F.col("radius").cast(IntegerType()))
)

# De-dupe based on business key (stop_id)
w = Window.partitionBy(F.col("stop_id")).orderBy(F.col("insert_time").desc())
df_dedupe = (
    df_transformed
...
# DQ checks
dq.check_not_empty(df_dedupe, "Staging stops")
dq.check_no_duplicates(df_dedupe, ["stop_id"], "Staging stops")
dq.check_values_in_set(df_dedupe, "wheelchair_boarding", [0, 1, 2], "Staging stops")
...
spark.sql(
    f"select * from hdw.job_control where table_name = '{table_name}' order by insert_dt desc limit 1"
).show(truncate=False)
